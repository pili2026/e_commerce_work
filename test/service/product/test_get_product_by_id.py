...
async def test_when_called_then_delegate_to_repository(product_service: ProductService):
    ...
    with patch.object(
        ...
        # Act
        result = await product_service.get_product(product_id=stub_get_product.id)

        # Assert
        assert result == stub_get_product
        mock_get_product.assert_called_once_with(product_id=stub_get_product.id, product_name=None)
